...
class WitAIService {
  ...
  Future<Map<String, dynamic>> processMessage(String message) async {
    ...
    try {
      ...
      return {
        "type": "text",
        "message":
        "Error: Unable to process your request. Please check your internet connection."
      };
    }
  }
  ...
  Future<Map<String, dynamic>> _processProductCode(
      Map<String, dynamic> entities) async {
    ...
    return {
      "type": "text",
      "message": "Could you please provide the product code?"
    };
  }
  ...
  Future<Map<String, dynamic>> _processProductDescription(
      Map<String, dynamic> entities) async {
    ...
    joinEntityValues(entities, "additional_component:additional_component");
    String colorDescription =
    joinEntityValues(entities, "color_description:color_description");
    String prodComponent =
    joinEntityValues(entities, "product_component:product_component");
    String prodModel =
    joinEntityValues(entities, "product_model:product_model");
    String prodName = joinEntityValues(entities, "product_name:product_name");
    String prodSeries =
    joinEntityValues(entities, "product_series:product_series");
    ...
    if (queryPart.isEmpty) {
      return {
        "type": "text",
        "message": "Could you please provide more details about the product?"
      };
    }
    ...
    try {
      ...
      // First, search for products available at Site.
      var siteQueryBuilder = supabase
          .from('products')
          .select('*')
          .eq('Location Type', 'Site')
          .gt('Quantity On Hand', 0);
      for (var token in tokens) {
        siteQueryBuilder = siteQueryBuilder.ilike('Concatenate', '%$token%');
      }
      final List<dynamic> siteResponse = await siteQueryBuilder;
      print("Supabase response (Site description search): $siteResponse");

      if (siteResponse.isNotEmpty) {
        List<dynamic> rankedProducts = _rankProducts(siteResponse, tokens);
        return {"type": "product", "products": rankedProducts};
      }

      // If no products at Site, search for products available at Van.
      var vanQueryBuilder = supabase
          .from('products')
          .select('*')
          .eq('Location Type', 'Van')
          .gt('Quantity On Hand', 0);
      for (var token in tokens) {
        vanQueryBuilder = vanQueryBuilder.ilike('Concatenate', '%$token%');
      }
      final List<dynamic> vanResponse = await vanQueryBuilder;
      print("Supabase response (Van description search): $vanResponse");

      if (vanResponse.isNotEmpty) {
        List<dynamic> rankedProducts = _rankProducts(vanResponse, tokens);
        return {"type": "product", "products": rankedProducts};
      } else {
        return {
          "type": "text",
  ...
  List<dynamic> _rankProducts(List<dynamic> products, List<String> tokens) {
    for (var product in products) {
      String concatText =
      (product['Concatenate'] ?? '').toString().toLowerCase();
      int score = 0;
      for (var token in tokens) {
    ...
    products.sort((a, b) => (b['matchScore'] as int)
        .compareTo(a['matchScore'] as int));
    print("Ranked products: ${products.map((p) => p['matchScore']).toList()}");
    return products;
  }

  /// Queries Supabase for a product with an exact product code.
  /// It first searches for products available at Site.
  /// If none are found, it searches for products available at Van.
  Future<Map<String, dynamic>> _checkProductAvailabilityByCode(
      String productCode) async {
    if (productCode.isEmpty) {
      return {
        "type": "text",
        "message": "❌ Product code is empty or malformed!"
      };
    }
    try {
      print("🔍 Searching for product with code: $productCode");

      // Search for products at Site.
      final List<dynamic> siteResponse = await supabase
          .from('products')
          .select('*')
          .eq('Product Code', productCode)
          .eq('Location Type', 'Site')
          .gt('Quantity On Hand', 0);
      print("Supabase response (Site code search): $siteResponse");

      if (siteResponse.isNotEmpty) {
        return {"type": "product", "products": siteResponse};
      }

      // If no product at Site, search for products at Van.
      final List<dynamic> vanResponse = await supabase
          .from('products')
          .select('*')
          .eq('Product Code', productCode)
          .eq('Location Type', 'Van')
          .gt('Quantity On Hand', 0);
      print("Supabase response (Van code search): $vanResponse");

      if (vanResponse.isNotEmpty) {
        return {"type": "product", "products": vanResponse};
      } else {
        return {
          "type": "text",
          "message":
  ...
  Map<String, dynamic> _generateBotResponse(
      String intent, Map<String, dynamic> entities) {
    switch (intent) {
      case "Greeting":
